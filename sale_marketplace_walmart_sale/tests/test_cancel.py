...
class TestCancel(TransactionCase):
    ...
    def test_cancel_posts_and_stamps(self):
        with (
            ...
            result = self.channel._walmart_cancel_order("PO-1", order=self.order)

        self.assertTrue(result)
        post_calls = [c for c in req.call_args_list if c.args[0] == "POST"]
        self.assertEqual(len(post_calls), 1)
        self.assertEqual(post_calls[0].args[1], "/v3/orders/PO-1/cancel")
        line = post_calls[0].kwargs["payload"]["orderCancellation"]["orderLines"][
            "orderLine"
        ][0]
        status = line["orderLineStatuses"]["orderLineStatus"][0]
        self.assertEqual(status["status"], "Cancelled")
        self.assertEqual(status["cancellationReason"], "CANCEL_BY_SELLER")
        self.assertEqual(status["statusQuantity"]["amount"], "2")
        self.assertTrue(self.order.walmart_cancelled)

    def test_cancel_idempotent(self):
        self.order.walmart_cancelled = fields.Datetime.now()
        with (
            patch(f"{_MODEL}._walmart_get_token", return_value="TOK"),
    ...
    def test_enqueue_on_odoo_cancel(self):
        with trap_jobs() as trap:
            self.order._action_cancel()
            trap.assert_jobs_count(1)

    def test_enqueue_skipped_when_auto_cancel_off(self):
        self.channel.walmart_auto_cancel = False
        with trap_jobs() as trap:
            self.order._action_cancel()
            trap.assert_jobs_count(0)

    def test_enqueue_skipped_when_already_cancelled(self):
        self.order.walmart_cancelled = fields.Datetime.now()
        with trap_jobs() as trap:
            self.order._action_cancel()
            trap.assert_jobs_count(0)

    def test_manual_button_cancels(self):
        with (
            patch(f"{_MODEL}._walmart_get_token", return_value="TOK"),
            patch(f"{_MODEL}._walmart_request", side_effect=self._fake_request()),
        ):
            self.order.action_walmart_cancel()
        self.assertTrue(self.order.walmart_cancelled)

    def test_non_walmart_cancel_does_not_enqueue(self):
        # cancelling a plain (non-Walmart) order must not enqueue or error
        plain = self.env["sale.order"].create(
            {
        ...
        with trap_jobs() as trap:
            plain._action_cancel()
            trap.assert_jobs_count(0)
        self.assertEqual(plain.state, "cancel")
